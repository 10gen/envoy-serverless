...
TEST(SubstitutionFormatterTest, ProxyProtocolTlvsFormatter) {
  ...
  // Success case - 2 TLV values of the associated type stored in the filter state, 1 TLV value with different type stored
  {
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    Network::ProxyProtocolTLV tlv_one{0x5, "\6\7"s};
    ...
    EXPECT_EQ("Bgc=, BgQ=", providers[0]->format(request_headers, response_headers, response_trailers,
                                           stream_info, body));
  }
}
