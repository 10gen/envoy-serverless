...
- serverless-proxy/al2023-x86_64
- serverless-proxy/al2023-aarch64
...
Usage:
  aws_ecr.sh --distro DISTRO [--build-docker-image] [--push-docker-image] [--pull-docker-image] [--run-script SCRIPT_PATH] [--mount-build-dir BUILD_DIR] [--mount-atlasproxy-dir ATLASPROXY_DIR] [--mount-ssh-dir SSH_DIR] [--interactive] [--detached] [--use-podman] [--print-trace] [--disable-seccomp]
  aws_ecr.sh -d DISTRO [-b] [-p] [-l] [-r SCRIPT_PATH] [-v BUILD_DIR] [-a ATLASPROXY_DIR] [-s SSH_DIR] [-i] [-t] [-u] [-x] [-c]

Options:
  -d, --distro                  Linux distribution that determines the Dockerfile used (al2, al2023). Required argument.
  ...
  -sa, --script-args             Specify additional bash shell arguments to pass into the script provided in the --run-script option. Optional argument.
  ...
  -t, --detached                Run script in detached mode. No-op if --run-script is absent. Optional argument.
  ...
  -c, --disable-seccomp         Disable secure computing mode (seccomp) when running Docker container (see https://docs.docker.com/engine/security/seccomp). Optional argument.
EOF
}
...
function run_script() {
  ...
  eval "${DOCKER_COMMAND}" run "$(${INTERACTIVE_MODE} && echo "-it")" "$(${DETACHED_MODE} && \
    ...
    "${EXTRA_MOUNTS}" "$(${DISABLE_SECCOMP} && echo "--security-opt seccomp=unconfined")" \
    "${ECR_REPO_URI}:${TAG}" /bin/bash -c "\"\"${CONTAINER_SCRIPT_PATH}\" ${SCRIPT_ARGS}\""
...
OPTIONS=$(getopt -a -l "help,distro:,build-docker-image,push-docker-image,pull-docker-image,run-script:,script-args:,mount-build-dir:,mount-atlasproxy-dir:,mount-ssh-dir:,interactive,detached,use-podman,print-trace,disable-seccomp" -o "hd:bplr:sa:v:a:s:ituxc" -- "$@")
eval set -- "${OPTIONS}"
...
DOCKER_COMMAND=$(${USE_PODMAN} && echo "podman --cgroup-manager cgroupfs" || echo "docker")
...
DOCKERFILE="${ROOT_DIR}/evergreen/Dockerfile-${DISTRO}"
